<?php include('header.php'); ?>


<div>
    <ul class="breadcrumb">
        <li>
            <a href="index.">Home</a> <span class="divider">/</span>
        </li>
        <li>
            <a href="pending_orders.php">Pending Orders</a>
        </li>
    </ul>
</div>


<br/><div class="row-fluid sortable">		
    <div class="box span12">
        <div class="box-header well" data-original-title>
            <h2><i class="icon-gift"></i> Details</h2>
            <!--<div class="box-icon">
                    <a href="#" class="btn btn-setting btn-round"><i class="icon-cog"></i></a>
                    <a href="#" class="btn btn-minimize btn-round"><i class="icon-chevron-up"></i></a>
                    <a href="#" class="btn btn-close btn-round"><i class="icon-remove"></i></a>
            </div>-->
        </div>

        <div class="box-content">
            <table class="table table-striped table-bordered bootstrap-datatable datatable">


                <tr>
                    <th>Name</th><td>Adam Mark</td></tr>
                <tr> <th>Email</th><td class="center">aa@gmail.com</td></tr>
                <tr> <th>College</th>	<td class="center">SVSIT</td></tr>
                <tr> <th>Contact Number</th> <td class="center">9538311396</td></tr>
                <tr> <th>Ordered Date</th> <td class="center">21/1/2013</td></tr>
                <tr> <th>Product Name</th> <td class="center">D-Signals monitoring</td></tr>
                <tr> <th>Model Number</th> <td class="center">MX345DF</td></tr>
                <tr> <th>Price</th> <td class="center">9999</td></tr>
                <tr> <th>Order Number</th> <td class="center">OD31231061105</td></tr>
                <tr> <th>Address</th> <td class="center">#1/1, 1st Block, RT Nagar Main Road, Opposite Axis Bank, Bangalore, Karnataka 560032
                        Bangalore, Karnataka
                        560032</td></tr>
                </tr>




<!--<tr> <th>Product Price</th><td class="center">8999Rs</td></tr>
<tr> <th>Project Kit Price</th><td class="center">10999Rs</td></tr>
<tr> <th>Do it yourself Kit Price</th><td class="center">11999Rs</td></tr>
<tr> <th>Readymade Kit Price</th><td class="center">9999Rs</td></tr>
<tr> <th>Product Price</th><td class="center">8999Rs</td></tr>
<tr> <th>Model Number</th> <td class="center">MX345DF</td></tr>
<tr> <th>Product Image1</th> <td class="center">images/img2</td></tr>
<tr> <th>Product Image2</th> <td class="center">images/img3</td></tr>
<tr> <th>Product Image3</th>  <td class="center">images/img4</td></tr>

<tr> <th>Download</th>  <td class="center">images/img6</td></tr>
<tr><th>Video Description</th> <td class="center">Abstact Full</td></tr>
<tr><th>Product Abstract</th> <td class="center">Abstact Full</td></tr>
<tr> <th>Product BlockDiagram</th><td class="center">images/img7</td></tr>-->
                <tr> <th>Actions</th><td class="center">
                        <a class="btn btn-success" href="#">
                            <i class="icon-zoom-in icon-white"></i>  
                            Print                                            
                        </a>


                        <a class="btn btn-info" href="#">
                            <i class="icon-edit icon-white"></i>  
                            Send Mail                                           
                        </a>


                    </td>


                </tr>






            </table>            
        </div>
    </div><!--/span-->

</div><!--/row-->

<div class="row-fluid sortable">
    <div class="box span12">
        <div class="box-header well" data-original-title>
            <h2><i class="icon-edit"></i> Change Status</h2>
            <!--<div class="box-icon">
                <a href="#" class="btn btn-setting btn-round"><i class="icon-cog"></i></a>
                <a href="#" class="btn btn-minimize btn-round"><i class="icon-chevron-up"></i></a>
                <a href="#" class="btn btn-close btn-round"><i class="icon-remove"></i></a>
            </div>-->
        </div>
        <div class="box-content">
            <form class="form-horizontal">
                <fieldset>
                    <div class="control-group">
                        <br/><label class="control-label" for="selectError">Change Status</label>
                        <div class="controls">
                            <select id="selectError" data-rel="chosen">
                                <option>Pending</option>
                                <option>Dispatched</option>
                                <option>Delivered</option>

                            </select>
                        </div>
                    </div>
                    <div class="form-actions">
                        <button type="submit" name="add" class="btn btn-primary">Save</button>

                    </div>





                </fieldset>
            </form>   

        </div>
    </div><!--/span-->

</div><!--/row-->
<?php include('footer.php'); ?>
